...
import { createFunction, minX, minY, Δx, Δy } from 'agora-graph';
...
var pfsPrime = createFunction(function (graph) {
  ...
  while (i < nodes.length) {
    ...
    if (u.x > lmin.x) {
      for (var m = i; m <= k; m++) {
        ...
        var ɣp = minX(v) + ɣpp < minX(lmin) ? σ : ɣpp;
        ɣ = Math.max(ɣ, ɣp);
      }
    }
  ...
  while (i < nodes.length) {
    ...
    if (u.y > lmin.y) {
      for (var m = i; m <= k; m++) {
        ...
        var ɣp = minY(v) + ɣpp < minY(lmin) ? σ : ɣpp;
        ɣ = Math.max(ɣ, ɣp);
      }
    }
